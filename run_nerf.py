...
sys.path.insert(0, "../")
from RawImage import ReadFrames
...
def train():
    ...
    if args.dataset_type == "llff":
        ...
        firstFrames = pickle.load(open("../firstFrames.pkl", "rb"))
        Intrinsics = pickle.load(open("../Intrinsics.pkl", "rb"))
        Extrinsics = pickle.load(open("../Extrinsics.pkl", "rb"))
        frames = ReadFrames(3800, firstFrames)

        frameNames = list(firstFrames.keys())

        images = torch.tensor(
            np.array([frames[name].astype(np.float32) / 255.0 for name in frameNames])
        )
        # world to camera
        poses_inv = np.array(
            [Extrinsics[name].astype(np.float32) for name in frameNames]
        )

        # camera to world convert
        poses = np.linalg.inv(poses_inv)[:, :3, :]
        ...
        # try 5 -- added poses inverse above
        poses[:, :, 1] *= -1
        poses[:, :, 2] *= -1

        K = torch.tensor(
            np.array([Intrinsics[name].astype(np.float32) for name in frameNames])
        )[0]
        ...
        hwf = [images.shape[1], images.shape[2], K[0, 0]]

        i_train = np.array([0, 2, 3, 5, 6, 8])
        i_val = np.array([1, 4, 7])
        i_test = np.array([1, 4, 7])
